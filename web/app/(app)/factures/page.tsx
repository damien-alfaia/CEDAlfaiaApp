...
function FilterTabs({ current, q }: { current: FacturesFilter; q?: string }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 rounded-md border bg-white p-1 text-sm">
      <span className="px-2 text-xs uppercase tracking-wider text-neutral-400">Filtre</span>
      {tabs.map((t) => (
        <Link
          key={t.key}
