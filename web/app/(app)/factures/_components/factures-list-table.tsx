...
export function FacturesListTable({ rows }: Props) {
  ...
  const printSelection = () => {
    ...
    window.open(`/factures/pdf-batch?ids=${ids}`, "_blank", "noopener,noreferrer");
  };
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="hidden text-right lg:table-cell">Reste</TableHead>
                ...
                <TableRow key={d.id} data-state={selected.has(d.id) ? "selected" : undefined}>
                  <TableCell className="w-10">
                    <Checkbox
                      ...
                      aria-label={`Sélectionner ${fmtNumero(d.num_facture)}`}
                    />
                  </TableCell>
                  ...
                  <TableCell className="hidden sm:table-cell">{fmtDate(d.date_facture)}</TableCell>
