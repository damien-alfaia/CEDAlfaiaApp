    ...
    public class PiecesVenteController : CEDAlfaiaAppControllerBase
    {
        ...
        private ActionResult saisirPieceVente(SaisirPieceVenteModel pPieceVente)
        {
            if (pPieceVente != null)
            {
                ...
                List<int> idsPaiements = new List<int>();
                List<int> idsPaiementsNewPieceVente = new List<int>();
                List<int> idsPaiementsASupprimer = new List<int>();
                    ...
                    if (!pPieceVente.IsValide)
                    {
                        ...
                        foreach (var service in pPieceVente.Services)
                        {
                            if (!service.IsEmpty && !service.IsSupprime)
                                CEDAlfaiaAppWebService.GetService().GarageService.AddUpdateServicePieceVente(service.IDService, pPieceVente.IDPieceVente, service.Libelle, service.PrixClientTTC / 1.2F, service.PrixClientTTC, service.Quantite);
                        ...
                        idsPaiements = facture.Paiements.Select(s => s.ID).ToList();
                        idsPaiementsNewPieceVente = pPieceVente.Paiements.Where(s => s.IDPaiement != 0).Select(s => s.IDPaiement).ToList();
                        idsPaiementsASupprimer = idsPaiements.Except(idsPaiementsNewPieceVente).ToList();
                ...
                {
                    // enregistrer devis
                    PieceVente devis = CEDAlfaiaAppWebService.GetService().GarageService.AddUpdateDevis(pPieceVente.IDPieceVente, pPieceVente.IDVoiture.Value, pPieceVente.Date, pPieceVente.MainDOeuvre, pPieceVente.DureeMainDOeuvre, pPieceVente.Kilometrage, pPieceVente.Remise, idRdv);
                    ...
                    idsPaiements = devis.Paiements.Select(s => s.ID).ToList();
                    idsPaiementsNewPieceVente = pPieceVente.Paiements.Where(s => s.IDPaiement != 0).Select(s => s.IDPaiement).ToList();
                    idsPaiementsASupprimer = idsPaiements.Except(idsPaiementsNewPieceVente).ToList();
                ...
                {
                    CEDAlfaiaAppWebService.GetService().GarageService.AddUpdatePieceVentePaiement(paiement.IDPaiement, pPieceVente.IDPieceVente, paiement.Date, paiement.Montant, paiement.TypePaiement);
                }

                foreach (var idPaiementASupprimer in idsPaiementsASupprimer)
                {
                    CEDAlfaiaAppWebService.GetService().GarageService.DeletePaiementPieceVente(idPaiementASupprimer);
                }
